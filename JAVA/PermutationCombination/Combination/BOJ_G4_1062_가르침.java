...
public class BOJ_G4_1062_가르침 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        //k<5면 못배움 => a n t i c 못읽으면 어차피 모든 단어 못읽음
        if (K<5) {
            MaxReadCnt = 0;
        } else {
    ...
    private static void chooseAlpha(int cnt, int idx, int isLearned) {
        ...
        if (idx==26) return;	//index 꽉차면 나가기
        switch(idx) {			//앞에서 미리 골라둔 알파벳은 그냥 넘어가기
            case 'a'-'a': case 'c'-'a': case 'i'-'a': case 'n'-'a': case 't'-'a':
                chooseAlpha(cnt, idx+1, isLearned);
                break;
            default: 				//앞에서 고른 알파벳이 아니라면
                isLearned = isLearned | 1<<idx;				//현재 idx의 알파벳 선택
                chooseAlpha(cnt+1, idx+1, isLearned);		//이번 알파벳 선택 했을 때
                isLearned = isLearned & ~(1<<idx);			//앞에서 선택해둔 거 다시 선택 안한 걸로 되돌리기
